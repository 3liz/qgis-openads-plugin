...
docker exec postgis bash -c "psql service=test -f /tests_directory/${PLUGIN_NAME}/tests/fixtures/install/sql/00_initialize_database.sql" > /dev/null
for sql_file in `ls -v ../${PLUGIN_NAME}/tests/fixtures/install/sql/${SCHEMA}/*.sql`; do
  echo "${sql_file}"
...
echo 'Generate doc'
docker exec postgis bash -c "apt-get update"
