...
docker exec postgis bash -c "psql service=test -f /tests_directory/${PLUGIN_NAME}/tests/data/install/sql/00_initialize_database.sql" > /dev/null
for sql_file in `ls -v ../${PLUGIN_NAME}/tests/data/install/sql/${SCHEMA}/*.sql`; do
  echo "${sql_file}"
